...
public class OpenCliAnalyzerProcessor(OptionProvider optionProvider)
{
    ...
    {
        IReadOnlyCollection<IOpenCliAnalyzer> analyzers = _analyzerSelector.GetAnalyzers(optionProvider);
        var diagnosticCollector = new DiagnosticCollector();
        var context = new OpenCliAnalyzeContext(document, optionProvider, diagnosticCollector);
